...
```
3. Submit the provided "job.jar" file to Flink using its web UI.

4. Populate Mock Enrichment cache, by running the "job.jar" on apache flink webapp using `pt.archifeed.flink.GenerateEnrichdata` as entry-point along with the following arguments:
```
...
6. Run the experiment by, on the apache Flink web UI using `pt.archifeed.flink.TransactionProcesser` as entry-point, with the following arguments:
